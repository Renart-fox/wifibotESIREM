...
{
    this->ui->videoContainer->removeWidget(videoStream);
    ...
    this->ui->blurBox->setChecked(false);
    this->ui->grayBox->setChecked(false);
    this->ui->negaBox->setChecked(false);
    this->ui->hueBox->setChecked(false);
}
...
void MainWindow::on_blurBox_stateChanged(int arg1)
{
    if(isBlurred)
    {
        isBlurred = !isBlurred;
        QString filters = getCurrentFilters();
        videoStream->page()->runJavaScript("document.body.firstChild.style.webkitFilter = '"+filters+"';");
    }
    else
    {
        isBlurred = !isBlurred;
        videoStream->page()->runJavaScript("var filtres = document.body.firstChild.style.webkitFilter; document.body.firstChild.style.webkitFilter = filtres+'blur(5px)';");
    }
}

void MainWindow::on_negaBox_stateChanged(int arg1)
{
    if(isNega)
    {
        isNega = !isNega;
    ...
    {
        isNega = !isNega;
        videoStream->page()->runJavaScript("var filtres = document.body.firstChild.style.webkitFilter; document.body.firstChild.style.webkitFilter = filtres+' invert(100%)';");
    }
}

void MainWindow::on_hueBox_stateChanged(int arg1)
{
    if(isInverted)
    {
    ...
    {
        isInverted = !isInverted;
        videoStream->page()->runJavaScript("var filtres = document.body.firstChild.style.webkitFilter; document.body.firstChild.style.webkitFilter = filtres+' hue-rotate(90deg)';");
    }
}

void MainWindow::on_grayBox_stateChanged(int arg1)
{
    if(isGray)
    {
        isGray = !isGray;
    ...
    {
        isGray = !isGray;
        videoStream->page()->runJavaScript("var filtres = document.body.firstChild.style.webkitFilter; document.body.firstChild.style.webkitFilter = filtres+' grayscale(100%)';");
    }
}

QString MainWindow::getCurrentFilters()
{
    QString str = "";
    if(isBlurred) str+="blur(5px) ";
    if(isGray) str+= "grayscale(100%) ";
    if(isNega) str+= "invert(100%) ";
